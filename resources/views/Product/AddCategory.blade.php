    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                <li class="breadcrumb-item"><a href="{{route('ProductCategory')}}"><i class="bx bx-home-alt"></i></a>
                </li>
...
<div class=" my-3 bg-white p-2">
    <form class="w-3/5 mx-auto" method="POST" action="{{route('ProductCategoryAdd')}}">
        @csrf
        <div class="my-2">
        ...
        <div class="my-2">
            <label for="">排序</label> <input type="text" name="sort" class="form-control my-2" value="{{old('sort')}}">
        </div>
        <div class="my-2">
            <label for="enable"><input type="radio" id="enable" name="enable" value="1" @if (old('enable')==1 ||
                    old('enable')==null ) checked @else @endif>
                啟用</label>
            <label for="no_enable"><input type="radio" id="no_enable" name="enable" value="0" @if(old('enable')!==null
                    && old('enable')==0) checked @endif>
                關閉</label>
        </div>
