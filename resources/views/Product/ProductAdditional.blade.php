                ...
                @foreach ($productAdditionals as $productAdditional)
                    <tr>
                        ...
                        <td class="text-center">
                            @if ($productAdditional->enable && date('Y-m-d h:i:s') >= $productAdditional->startTime && date('Y-m-d h:i:s') < $productAdditional->endTime)
                                <span class="bg-success p-1 rounded text-white">啟用中</span>
                            @elseif($productAdditional->enable && date('Y-m-d h:i:s') < $productAdditional->startTime)
                                <span class="bg-danger p-1 rounded text-white">尚未開始</span>
                            @elseif($productAdditional->enable && date('Y-m-d h:i:s') > $productAdditional->endTime)
                                <span class="bg-danger p-1 rounded text-white">已過期</span>
                            @else
                        ...
                        <td class="text-center">
                            ...
                            <div class="modal fade" id="deleteModal{{ $productAdditional->productAdditionId }}"
