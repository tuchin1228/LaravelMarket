    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">新增品牌介紹</li>
            </ol>
        </nav>
    </div>
</div>
...
<div class=" my-3 bg-white p-2">
    <div class="">
        ...
        <form class="w-3/5 mx-auto" method="POST" action="{{route('UploadAbout')}}" enctype="multipart/form-data">
            ...
            <input type="text" hidden name="about_id" value="">
            ...
            <div class="col-12">
                <label for="" class="d-block mt-4">是否顯示於首頁{{old('showHome')}}</label>
                <select class="form-select  my-2 " name="showHome" id="">
                    <option value="1" @if( empty(old('showHome')) || (!empty(old('showHome')) && old('showHome')==1) )
                        selected @endif>
                        是</option>
                    <option value="0" @if(!empty(old('showHome') && old('showHome')==0)) selected @endif>否</option>
                </select>
            </div>
...
<script>
    const about_id = Math.floor(Date.now() / 1000);
